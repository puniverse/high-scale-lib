...
public class perf_set_test extends Thread {
  ...
  static final String _names[] = {"All", "HashSet", "NBHashSet","NBSetInt"};
  ...
  static void run( int num_threads, int num_trials, int impl ) throws Exception {
    if( impl == 0 ) {
  ...
  static void run_till_stable( int num_threads, int num_trials, int impl ) throws Exception {
    Set<Integer> AS;
    switch( impl ) {
    case 1: AS = new HashSet<Integer>(); break;
  ...
  static long run_once( int num_threads, Set AS, long[] ops, long [] nanos ) throws Exception {
    Random R = new Random();
    _start = false;
    _stop = false;
      ...
      while( true ) {
        ...
        if( trip++ > 1024*1024 ) {
          if( trip > 1024*1024+100 ) 
            throw new Exception("barf trip "+sz+" "+AS.size()+" numkeys="+KEYMAX);
  ...
  public void run() {
    if( _read_ratio == 0 ) {
      ...
      else                                    run_normal_int ((AbstractSet<Integer>)_set);
    }
  }
  ...
  // old entries.
  public void run_churn_int( AbstractSet<Integer> as ) {
  ...
  public void run_normal_int( AbstractSet<Integer> as ) {
    SimpleRandom R = new SimpleRandom();
    while( !_start )            // Spin till Time To Go
      try { Thread.sleep(1); } catch( Exception e ){}
